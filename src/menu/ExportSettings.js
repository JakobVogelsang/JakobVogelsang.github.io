...
function getSDI(sdi) {
  ...
  if (!parent || parent.tagName !== "SDI")
    return [sdi];
  return getSDI(parent);
}
function getReference(dai) {
  let reference = dai.getAttribute("name") ?? "";
  ...
  reference = (anyLn.getAttribute("prefix") ?? "") + (anyLn.getAttribute("lnClass") ?? "") + (anyLn.getAttribute("lnInst") ?? "") + "/" + reference;
...
function getTypeElement(dai) {
  const dataTypeTempltes = dai.closest("SCL")?.querySelector("DataTypeTemplates");
  if (!dataTypeTempltes)
    return null;
  ...
  if (!anyln || !doi)
    return null;
  const dO = dataTypeTempltes.querySelector(`LNodeType[id="${anyln.getAttribute("lnType")}"] > DO[name="${doi.getAttribute("name")}"]`);
  let doTypeOrDaType = dataTypeTempltes.querySelector(`DOType[id="${dO?.getAttribute("type")}"]`);
  for (const sdi of sdis) {
    const sDOorDAorBDA = doTypeOrDaType?.querySelector(`SDO[name="${sdi?.getAttribute("name")}"], DA[name="${sdi?.getAttribute("name")}"], BDA[name="${sdi?.getAttribute("name")}"]`);
    doTypeOrDaType = dataTypeTempltes.querySelector(`DOType[id="${sDOorDAorBDA?.getAttribute("type")}"], DAType[id="${sDOorDAorBDA?.getAttribute("type")}"]`);
  }
  return doTypeOrDaType?.querySelector(`DA[name="${dai.getAttribute("name")}"], BDA[name="${dai.getAttribute("name")}"]`) ?? null;
}
function isSetting(typeElement) {
  if (!typeElement)
    return false;
  return settingTypeFunctionalContrains.includes(typeElement.getAttribute("fc"));
}
function filterSettings(doc) {
  const setting = [[]];
